...
const register = (policyOptions) => {
  ...
  policyOptions.policy = (params, ...args) => {
    ...
    log.error(`Policy ${chalk.default.red.bold(name)} params validation failed: ${validationResult.error}`);
...
const resolve = (policyName) => {
  ...
  if (!policy) {
    log.error(`Could not find policy ${policyName}, Please make sure the plugins providing such policy
...
const load = async (policiesToBeLoaded) => {
  ...
  for (const name of corePolicies) {
    if (!policies[name]) {
      log.debug(`registering policy ${chalk.green(name)}`);
